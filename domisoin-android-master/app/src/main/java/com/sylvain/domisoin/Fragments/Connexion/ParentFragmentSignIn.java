...
public class ParentFragmentSignIn extends Fragment implements View.OnClickListener, ViewPager.OnPageChangeListener {
    ...
    private Spinner profession_s = null;
    ...
    private void getAlldata() {
        ...
        if (!pro_patient) {
            ...
            datas.put("speciality", String.valueOf(String.valueOf(profession_s.getSelectedItem())));
        ...
        switch (position) {
            case 0:
                if (ParentFragmentSignIn.pro_patient) {
                   ...
                   // profession_s = (Spinner)view.findViewById(R.id.spec_pro);

                    if (last_name.getText().toString().length() == 0)
                        last_name.setError("Un Nom de famille valide est requis");
                        // avant c'était comme ça   profession_s.getContext().toString().length()
               //     else if( profession_s.getSelectedItem().toString().length() == 0 )  //( job.getText().toString().length() == 0 )
                  //      profession_s.setError("adapter");
